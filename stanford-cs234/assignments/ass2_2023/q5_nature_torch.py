...
class NatureQN(Linear):
    ...
    def initialize_models(self):
        ...
        self.q_network = torch.nn.Sequential(
            torch.nn.Conv2d(in_channels = n_channels*self.config.state_history, out_channels = 16, kernel_size = 3, stride = 1),  # One convolution layer with 16 output channels, a kernel size of 3, stride 1, and no padding.
            torch.nn.ReLU(),  # ReLU Activation
            torch.nn.Flatten(),  # persist batch dimension
            torch.nn.Linear(in_features = 8 * 8 * 16, out_features = 128), #128 hidden
            torch.nn.ReLU(),
            torch.nn.Linear(in_features = 128, out_features = num_actions),  # in_features=512
        )
