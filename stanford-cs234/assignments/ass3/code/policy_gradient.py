...
class PolicyGradient(object):
    ...
    def init_policy(self):
        ...
        policy_network = build_mlp(input_size = self.observation_dim, output_size = self.action_dim, n_layers = self.config.n_layers, size = self.config.layer_size)
        if self.discrete:
            self.policy = CategoricalPolicy(policy_network).to(device)
        else:
            self.policy = GaussianPolicy(policy_network, self.action_dim).to(device)
    ...
    def get_returns(self, paths):
        ...
        for path in paths:
            ...
            T = len(rewards)
            discounted_rewards = self.config.gamma ** np.arange(T) * rewards
