...
class BaselineNetwork(nn.Module):
    ...
    def __init__(self, env, config):
        """
        TODO:
        Create self.baseline using build_mlp, and create self.optimizer to
        ...
        self.baseline = build_mlp(input_size = input_dim, output_size = 1, n_layers = self.config.n_layers, size=self.config.layer_size)
        self.optimizer = torch.optim.Adam(params=self.baseline.parameters(), lr=self.lr)
        #######################################################
        #########          END YOUR CODE.          ############
    ...
    def calculate_advantage(self, returns, observations):
        ...
        baseline = self.forward(observations).cpu().detach().numpy()
    ...
    def update_baseline(self, returns, observations):
        ...
        loss = F.mse_loss(baselines, returns)
        loss.backward()
        self.optimizer.step()
        #######################################################
        #########          END YOUR CODE.          ############
