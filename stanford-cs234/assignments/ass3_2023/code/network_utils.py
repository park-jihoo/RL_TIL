...
def build_mlp(input_size, output_size, n_layers, size):
    ...
    model = nn.Sequential(
        nn.Linear(in_features=input_size, out_features=size, device=device),
        nn.ReLU()
    )

    for i in range(n_layers - 1):
        model.append(nn.Linear(in_features=size, out_features=size, device=device))
        model.append(nn.ReLU())

    model.append(nn.Linear(in_features=size, out_features = output_size, device=device))
    return model
    #######################################################
    #########          END YOUR CODE.          ############
