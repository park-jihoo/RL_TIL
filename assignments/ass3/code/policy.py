...
class BasePolicy:
    ...
    def act(self, observations):
        ...
        sampled_actions = self.action_distribution(observations).sample().cpu().detach().numpy()
        #######################################################
        #########          END YOUR CODE.          ############
...
class GaussianPolicy(BasePolicy, nn.Module):
    ...
    def action_distribution(self, observations):
        ...
        distribution = torch.distributions.MultivariateNormal(loc=loc, scale_tril=scale)
        #######################################################
        #########          END YOUR CODE.          ############
        return distribution
